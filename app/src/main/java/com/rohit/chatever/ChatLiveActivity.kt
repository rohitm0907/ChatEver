...
import com.rohit.chatever.MyConstants.FIREBASE_BASE_URL
import com.rohit.chatever.adapters.ChatLiveAdapter
import com.rohit.chatever.databinding.ActivityChatLiveBinding
import kotlinx.android.synthetic.main.activity_chat_live.*
...
class ChatLiveActivity : AppCompatActivity() {
    ...
    private var audioRecorder: com.rohit.chatever.AudioRecorder? = null
    ...
    var firebaseUsers =
        FirebaseDatabase.getInstance(com.rohit.chatever.MyConstants.FIREBASE_BASE_URL)
            .getReference(com.rohit.chatever.MyConstants.NODE_USERS)
    var firebaseLikedUsers =
        FirebaseDatabase.getInstance(com.rohit.chatever.MyConstants.FIREBASE_BASE_URL)
            .getReference(com.rohit.chatever.MyConstants.NODE_LIKED_USERS)
    var firebaseChatFriends =
        FirebaseDatabase.getInstance(FIREBASE_BASE_URL)
            .getReference(com.rohit.chatever.MyConstants.NODE_CHAT_FIRENDS)
    ...
    companion object {
        private var instance: com.rohit.chatever.ChatLiveActivity? = null

        fun getInstance(): com.rohit.chatever.ChatLiveActivity? {
            return com.rohit.chatever.ChatLiveActivity.Companion.instance
        }
    ...
    fun onBlock(name: String) {
        ...
        if (intent.getStringExtra(com.rohit.chatever.MyConstants.OTHER_USER_NAME).equals(name)) {
            finish()
        }
    }
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (!intent.getStringExtra(com.rohit.chatever.MyConstants.OTHER_USER_IMAGE).equals("")) {
            Glide.with(this@ChatLiveActivity)
                .load(intent.getStringExtra(com.rohit.chatever.MyConstants.OTHER_USER_IMAGE))
                .placeholder(com.rohit.chatever.R.drawable.user)
                .into(binding!!.imgUser)
        }
        clicks()
        senderId = com.rohit.chatever.MyUtils.getStringValue(
        ...
        binding!!.imgLike.setOnClickListener {
            if (isLikedProfile == false) {
                ...
                binding!!.imgLike.setImageResource(com.rohit.chatever.R.drawable.ic__liked)
                if (!token.equals("")) {
                    com.rohit.chatever.Firebase.FirebaseNotification.MyNotification.sendNotification(
                        com.rohit.chatever.MyUtils.getStringValue(
                            this,
                            com.rohit.chatever.MyConstants.USER_NAME
                        ).toString(),
                        "has liked your profile.",
                        token,
                        com.rohit.chatever.MyConstants.NOTI_REQUEST_TYPE,
                        roomId
                    )
                }
                isLikedProfile = true
            } else {
                com.rohit.chatever.MyUtils.showToast(this, "Already liked")
            }
        }
            ...
            .addListenerForSingleValueEvent(object : ValueEventListener {
                override fun onDataChange(snapshot: DataSnapshot) {
                    if (snapshot.exists()) {
                        ...
                        binding!!.imgLike.setImageResource(com.rohit.chatever.R.drawable.ic__dislike)
                    }
                ...
                when (which) {
                    ...
                    2 -> {
                        var location = com.rohit.chatever.MyUtils.getStringValue(
                            this@ChatLiveActivity,
                            com.rohit.chatever.MyConstants.USER_LATITUDE,
                        ) + "," + "" + com.rohit.chatever.MyUtils.getStringValue(
                            this@ChatLiveActivity,
                            com.rohit.chatever.MyConstants.USER_LONGITUDE,
                        )
...
//        var file=File(videoUrl)
        val mountainvideosRef: StorageReference =
            storageRef.child("audios/" + "android" + Calendar.getInstance().time + ".3gp")
    ...
    private fun sendMessageOnFirebase(message: String?, messageType: String) {
        ...
        firebaseChats.child(roomId!!).child(key.toString()).setValue(data)
            .addOnCompleteListener {
                com.rohit.chatever.MyUtils.stopProgress(this@ChatLiveActivity)
...
//                )
                ...
                data.put(
                    "name", com.rohit.chatever.MyUtils.getStringValue(
                        this@ChatLiveActivity,
                        com.rohit.chatever.MyConstants.USER_NAME
                    )
                )
                ...
                data.put(
                    "image", com.rohit.chatever.MyUtils.getStringValue(
                        this@ChatLiveActivity,
                        com.rohit.chatever.MyConstants.USER_IMAGE
                    ).toString()
                )
...
//                        Toast.makeText(this@ChatLiveActivity,"send",Toast.LENGTH_SHORT).show()
                ...
                if (chatsList.size == 0 || chatsList.size == 1) {
                    if (intent.getStringExtra(com.rohit.chatever.MyConstants.FROM) == null) {
                        if (!com.rohit.chatever.MyUtils.listFriends.contains(receiverId)) {
                            com.rohit.chatever.MyUtils.listFriends.add(receiverId)
                        }
                    }
        ...
        PermissionX.init(this@ChatLiveActivity)
            ...
            .request { allGranted, grantedList, deniedList ->
                if (allGranted) {
                    ...
                    audioRecorder = com.rohit.chatever.AudioRecorder()
...
//        firebaseChats.removeEventListener(stateValueEventListner!!);
    ...
    fun getFormattedDate(timeInMilli: Long): String? {
